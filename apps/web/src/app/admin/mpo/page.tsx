...
export default function AdminMpoManagementPage() {
  ...
  return (
    <div className="space-y-8">
      {/* Header */}
      <div className="p-6 rounded-3xl border border-slate-200 bg-white flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <div className="flex items-center gap-2 text-sky-600 text-xs font-bold uppercase tracking-wider font-mono">
            <Briefcase className="w-4 h-4" /> Confidential Sector HQ
          </div>
          <h1 className="text-2xl font-bold text-slate-900 mt-1">MPO Market & Territories</h1>
        ...
        <button
          ...
          className="px-4 py-2.5 rounded-xl bg-sky-500 text-white font-bold text-xs flex items-center gap-2 hover:bg-sky-400 transition-all shadow-lg "
        >
        ...
        <button
          ...
          className={`px-4 py-2 rounded-xl text-xs font-bold transition-all flex items-center gap-2 ${
            activeTab === 'territories'
              ? 'bg-slate-100 text-sky-600 border border-slate-200'
                ...
                <div className="flex justify-between items-start">
                  <div>
                    <div className="flex items-center gap-2">
                      ...
                      <h3 className="text-base font-bold text-slate-900">{group.territory}</h3>
                    </div>
                    <div className="text-[11px] text-slate-500 mt-0.5">
                      {group.mpoCount} Assigned MPO{group.mpoCount > 1 ? 's' : ''} (Auto-Grouped)
                    </div>
                  </div>
                  <span className="px-2.5 py-1 rounded-full text-[10px] font-mono bg-sky-50 text-sky-600 border border-sky-200">
                    Active Territory
                  </span>
                </div>
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-3">
            {queue.map((item) => (
              <div
                key={item.id}
                className="p-5 rounded-2xl border border-slate-200 bg-white flex flex-col md:flex-row justify-between items-start md:items-center gap-4 text-xs"
              >
                <div className="space-y-1">
                  <div className="flex items-center gap-2">
                    <span className="font-mono text-sky-600 font-bold">{item.listingNumber}</span>
                  ...
                  <div className="text-slate-500 text-[11px] flex gap-4">
                    ...
                    <span>MPO Target: <strong className="text-amber-700 font-mono">৳{item.mpoTargetPrice}</strong></span>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className="px-3 py-1.5 rounded-xl bg-sky-50 text-sky-600 border border-sky-200 font-bold hover:bg-sky-50 transition-all"
                  >
                    Configure & Review
                  </button>
                </div>
      ...
      {selectedListing && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="max-w-lg w-full p-6 rounded-3xl border border-slate-200 bg-white space-y-4">
            ...
            <div className="space-y-3 text-xs">
              <div className="p-3 bg-slate-100/40 rounded-xl space-y-1">
                <div className="text-slate-900 font-bold">{selectedListing.productName}</div>
              ...
              <div className="space-y-2 pt-2">
                <label className="text-slate-600 font-bold block">Wholesale Channel Visibility & Unit Price (BDT)</label>
                <div className="flex items-center gap-3">
                  ...
                  />
                  <span className="text-slate-500">Visible to Wholesalers</span>
                  <input
                    type="number"
                    value={reviewForm.wholesaleUnitPrice}
                    onChange={(e) => setReviewForm({ ...reviewForm, wholesaleUnitPrice: parseFloat(e.target.value) })}
                    className="w-28 px-2 py-1 rounded bg-slate-100 border border-slate-200 text-slate-900 font-mono"
                    placeholder="Price ৳"
                  />
                </div>
              </div>
              ...
              <div className="space-y-2 pt-2">
                <label className="text-slate-600 font-bold block">Paikari Channel Visibility & Unit Price (BDT)</label>
                <div className="flex items-center gap-3">
                  <input
                    type="checkbox"
                    ...
                    className="rounded text-sky-500 focus:ring-0"
                  />
                  <span className="text-slate-500">Visible to Paikari</span>
                  <input
                    type="number"
            ...
            <div className="flex justify-end gap-2 pt-4 border-t border-slate-200">
              <button
                ...
                className="px-3 py-1.5 rounded-xl text-slate-500 hover:text-slate-700 font-bold text-xs"
              >
                Cancel
              </button>
              <button
                onClick={() => handleReviewListing(selectedListing.id, 'REJECTED')}
                className="px-3 py-1.5 rounded-xl bg-rose-50 text-rose-600 border border-rose-500/20 font-bold text-xs hover:bg-rose-50"
              >
                Reject
              </button>
              <button
      ...
      {showCreateModal && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <form
            ...
            className="max-w-lg w-full p-6 rounded-3xl border border-slate-200 bg-white space-y-4"
          >
            <h3 className="text-base font-bold text-slate-900 flex items-center gap-2">
              <Shield className="w-4 h-4 text-sky-600" /> Admin Direct MPO Account Creation
            </h3>
            <p className="text-xs text-slate-500">
            ...
            <div className="space-y-3 text-xs">
              <div>
                <label className="text-slate-600 font-bold block mb-1">Full Representative Name</label>
                <input
                  type="text"
                  required
              ...
              <div>
                <label className="text-slate-600 font-bold block mb-1">Assigned Territory (Auto-Grouping)</label>
                <input
                  type="text"
                  required
              ...
              <div>
                <label className="text-slate-600 font-bold block mb-1">Private Admin Notes</label>
                <textarea
                  rows={2}
