...
export default function AdminPreOrdersPage() {
  ...
  const getStatusBadge = (status: PreOrderStatus) => {
    switch (status) {
      case PreOrderStatus.PENDING:
        return <span className="px-2.5 py-1 rounded-full bg-amber-500/20 border border-amber-500/40 text-amber-300 text-[11px] font-mono font-semibold">PENDING SOURCING</span>;
      case PreOrderStatus.SOURCING:
        return <span className="px-2.5 py-1 rounded-full bg-sky-500/20 border border-sky-500/40 text-sky-300 text-[11px] font-mono font-semibold">SOURCING / MPO ASSIGNED</span>;
      case PreOrderStatus.CONFIRMED:
        return <span className="px-2.5 py-1 rounded-full bg-indigo-500/20 border border-indigo-500/40 text-indigo-300 text-[11px] font-mono font-semibold">STOCK CONFIRMED</span>;
      case PreOrderStatus.FULFILLED:
        return <span className="px-2.5 py-1 rounded-full bg-emerald-500/20 border border-emerald-500/40 text-emerald-300 text-[11px] font-mono font-semibold">FULFILLED</span>;
      case PreOrderStatus.CANCELLED:
        return <span className="px-2.5 py-1 rounded-full bg-slate-800 border border-slate-700 text-slate-400 text-[11px] font-mono font-semibold">CANCELLED</span>;
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="p-6 rounded-3xl glass-panel border border-indigo-500/30 bg-gradient-to-r from-indigo-500/10 via-slate-900 to-slate-950 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-bold text-white">Wholesale Pre-Order Demand Backlog</h1>
      ...
      <div className="glass-panel p-4 rounded-3xl border border-slate-800 bg-slate-900/80 flex flex-col md:flex-row gap-4 items-center justify-between">
        ...
        <div className="flex flex-wrap items-center gap-3 w-full md:w-auto">
          ...
          <div className="flex items-center gap-1.5 text-xs bg-slate-950 px-3 py-2 rounded-xl border border-slate-800">
            ...
            >
              <option value="ALL" className="bg-slate-900">All Lead Times</option>
              <option value="2" className="bg-slate-900">2 Days (Urgent)</option>
              <option value="3" className="bg-slate-900">3 Days</option>
              <option value="4" className="bg-slate-900">4 Days</option>
      ...
      {/* Pre-Orders List */}
      {loading ? (
        <div className="text-center py-12 text-slate-400 text-xs">Loading pre-order demand backlog...</div>
      ) : filteredPreOrders.length === 0 ? (
        <div className="glass-panel p-12 rounded-3xl border border-slate-800 text-center space-y-2">
          <Package className="w-8 h-8 text-slate-600 mx-auto" />
          <div className="text-sm font-bold text-slate-300">No Pre-Orders in this queue</div>
        ...
        <div className="space-y-4">
          {filteredPreOrders.map((po) => (
            <div
              key={po.id}
              className="glass-panel p-5 rounded-3xl border border-slate-800 bg-slate-900/80 hover:border-slate-700 transition-all space-y-4"
            >
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-800/60 pb-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <span className="text-xs font-mono text-slate-400 font-semibold">{po.preOrderNumber}</span>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-xs">
                ...
                <div className="p-3 rounded-2xl bg-slate-950/70 border border-slate-800 space-y-1.5">
                  ...
                  <div className="text-[11px] text-indigo-400 font-mono">{po.genericName}</div>
                  <div className="text-[11px] text-slate-400">{po.companyName}</div>
                </div>
                ...
                <div className="p-3 rounded-2xl bg-slate-950/70 border border-slate-800 space-y-1.5">
                  ...
                  <div className="flex items-baseline gap-2">
                    <span className="text-lg font-bold text-emerald-400 font-mono">{po.requestedQuantity}</span>
                    <span className="text-slate-400 font-medium">{po.unitType}s</span>
                  </div>
                  {po.targetPrice && (
                    <div className="text-[11px] text-slate-300">
                      Target Whl Rate: <span className="font-mono font-bold text-amber-300">৳{po.targetPrice.toFixed(2)}</span>
                    </div>
                  )}
                  {po.notes && (
                    <div className="text-[11px] text-slate-400 italic bg-slate-900/80 p-1.5 rounded-lg">
                      "{po.notes}"
                    </div>
                  )}
                </div>
              ...
              <div className="flex flex-wrap items-center justify-end gap-2 pt-2 border-t border-slate-800/40">
                ...
                {po.status !== PreOrderStatus.CANCELLED && po.status !== PreOrderStatus.FULFILLED && (
                  <button
                    ...
                    className="px-3 py-1.5 rounded-xl bg-slate-800 hover:bg-rose-900/40 text-slate-400 hover:text-rose-300 font-semibold text-xs transition-colors"
                  >
                    Cancel Request
                  </button>
                )}
