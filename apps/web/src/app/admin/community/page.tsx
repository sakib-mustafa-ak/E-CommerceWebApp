    ...
    <div className="space-y-6">
      ...
      {activeTab === 'PENDING' && (
        <div className="space-y-4">
          {pendingPosts.length === 0 ? (
            <div className="p-12 text-center rounded-3xl border border-slate-800 bg-slate-900/40 text-slate-500">
            ...
            <div className="grid grid-cols-1 gap-4">
              {pendingPosts.map((post) => (
                <div
                  key={post.id}
                  className="p-6 rounded-3xl border border-slate-800 bg-slate-900/80 backdrop-blur-md shadow-xl flex flex-col md:flex-row items-start md:items-center justify-between gap-6"
                >
                  <div className="space-y-2.5 flex-1">
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      <span className="px-2.5 py-0.5 rounded-full bg-indigo-500/20 text-indigo-300 text-[11px] font-semibold">
                    ...
                    <p className="text-xs text-slate-300 line-clamp-2 leading-relaxed whitespace-pre-wrap">
                      {post.content}
                    </p>

                    <div className="flex flex-wrap items-center gap-4 text-xs text-slate-400 pt-1">
                      <span className="text-white font-medium">Author: {post.authorName}</span>
                      {post.authorPhone && (
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    ...
                    <button
                      ...
                      className="px-4 py-2.5 rounded-xl bg-rose-600/20 hover:bg-rose-600/30 text-rose-300 border border-rose-500/40 font-bold text-xs transition-all flex items-center gap-1.5"
                    >
                ...
                {allPosts.map((post) => (
                  <tr key={post.id} className="text-slate-300">
                    <td className="py-3">
                      ...
                      <div className="text-[11px] text-slate-400">{post.authorName}</div>
                    </td>
                    <td className="py-3">
                    ...
                    <td className="py-3 text-right">
                      {post.status === CommunityPostStatus.APPROVED && (
                        <button
                          ...
                          className="px-3 py-1.5 rounded-lg bg-rose-950 hover:bg-rose-900 text-rose-300 border border-rose-800 text-xs font-semibold"
                        >
                          Remove Post
                        </button>
                      )}
      ...
      {rejectingPostId && (
        <div className="fixed inset-0 z-50 bg-slate-950/80 backdrop-blur-md flex items-center justify-center p-4">
          <div className="w-full max-w-md p-6 rounded-3xl border border-slate-800 bg-slate-900 space-y-4 shadow-2xl">
            <h3 className="font-bold text-base text-white flex items-center gap-2 text-rose-400">
