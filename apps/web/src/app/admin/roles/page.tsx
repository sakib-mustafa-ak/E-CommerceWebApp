...
export default function RolesAdminPage() {
  ...
  return (
    <div className="space-y-8">
      <div>
        <h1 className="text-xl font-bold text-slate-900">Dynamic Staff Roles & Permission Matrix</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {roles.map((role) => (
          <div
            key={role.id}
            className="p-5 rounded-2xl border border-slate-200 bg-white flex flex-col justify-between space-y-4"
          >
            <div>
              ...
              <p className="text-xs text-slate-500 mt-1">{role.description || 'Custom administrative staff role'}</p>
            </div>
              ...
              <div className="flex flex-wrap gap-1 max-h-24 overflow-y-auto">
                {role.rolePermissions?.map((rp: any) => (
                  <span
                    ...
                    className="text-[10px] px-2 py-0.5 rounded-md bg-slate-50 border border-slate-200 text-sky-700 font-mono"
                  >
                    {rp.permission?.slug}
                  </span>
                ))}
      ...
      <div className="p-6 rounded-3xl border border-slate-200 bg-white space-y-6">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-base font-bold text-slate-900">Create New Custom Staff Role</h2>
        </div>

        <form onSubmit={handleCreateRole} className="space-y-6">
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div>
              <label className="block text-xs text-slate-600 mb-1">Role Title *</label>
              <input
                type="text"
                required
                value={newRoleName}
            ...
            <div>
              <label className="block text-xs text-slate-600 mb-1">Role Slug (System Key) *</label>
              <input
                type="text"
                required
                value={newRoleSlug}
                ...
                className="w-full px-3.5 py-2 bg-slate-50 border border-slate-200 rounded-xl text-xs text-sky-700 font-mono"
              />
            </div>
            <div>
          ...
          <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2.5 p-4 rounded-2xl bg-slate-50 border border-slate-200">
              {permissions.map((p) => {
                ...
                return (
                  <button
                    ...
                    className={`p-2.5 rounded-xl border text-left flex items-start gap-2.5 transition-colors ${
                      isChecked
                        ? 'border-sky-500 bg-sky-50 text-sky-200'
                        : 'border-slate-200 bg-white text-slate-500 hover:border-slate-200'
                    }`}
                  >
                    {isChecked ? (
          ...
          <button
            type="submit"
            disabled={loading}
            className="py-2.5 px-6 rounded-xl bg-sky-500 hover:bg-sky-400 text-white font-bold text-xs shadow-lg  transition-colors"
          >
