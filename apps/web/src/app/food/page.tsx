...
export default function FoodMarketplacePage() {
  ...
  return (
    <div className="min-h-screen pb-20 bg-slate-950 text-slate-100">
      {/* Hero Banner */}
      <div className="relative overflow-hidden bg-gradient-to-b from-rose-950/40 via-slate-900 to-slate-950 border-b border-rose-900/30 pt-10 pb-16">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
            ...
            <div className="flex flex-col sm:flex-row gap-3 w-full md:w-auto">
              <Link
                href="/food/vendor"
                className="px-5 py-3 rounded-2xl bg-gradient-to-r from-rose-600 to-amber-600 hover:from-rose-500 hover:to-amber-500 text-white font-bold text-sm shadow-lg shadow-rose-900/30 flex items-center justify-center gap-2 transition-all transform active:scale-95"
              >
          ...
          <div className="mt-8 grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="md:col-span-2 relative">
              ...
              <input
                ...
                className="w-full pl-12 pr-4 py-3.5 bg-slate-900/90 border border-slate-700/80 rounded-2xl text-white placeholder-slate-400 focus:outline-none focus:border-rose-500 focus:ring-1 focus:ring-rose-500 transition-all text-sm"
              />
            </div>

            {/* Area Selector */}
            <div className="relative">
              <MapPin className="w-5 h-5 absolute left-4 top-1/2 -translate-y-1/2 text-rose-400" />
              <select
                value={selectedArea}
                onChange={(e) => setSelectedArea(e.target.value)}
                aria-label="Filter restaurants by Dhaka area"
                className="w-full pl-12 pr-4 py-3.5 bg-slate-900/90 border border-slate-700/80 rounded-2xl text-white focus:outline-none focus:border-rose-500 transition-all text-sm appearance-none cursor-pointer"
              >
                {DHAKA_AREAS.map((a) => (
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-8">
        ...
        {restaurants.length === 0 ? (
          <div className="p-12 text-center rounded-3xl border border-slate-800 bg-slate-900/40 space-y-3">
            ...
            <p className="text-slate-300 font-semibold">No restaurants match your search criteria.</p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {restaurants.map((restaurant) => (
              <Link
                ...
                className="group rounded-3xl border border-slate-800 hover:border-rose-500/50 bg-slate-900/80 hover:bg-slate-900 overflow-hidden flex flex-col transition-all duration-300 hover:shadow-xl hover:shadow-rose-950/20"
              >
                {/* Banner Thumbnail */}
                <div className="relative h-44 w-full bg-slate-950 overflow-hidden">
                  ...
                  {/* Area Badge */}
                  <div className="absolute top-3 left-3 px-3 py-1 rounded-full bg-slate-950/80 backdrop-blur-md border border-slate-800 text-white text-[11px] font-semibold flex items-center gap-1.5">
                    <MapPin className="w-3 h-3 text-rose-400" />
                    {restaurant.area}
                  </div>

                  {/* Open / Closed Status */}
                  <div className="absolute top-3 right-3 px-3 py-1 rounded-full backdrop-blur-md text-[11px] font-semibold flex items-center gap-1.5 bg-emerald-950/80 border border-emerald-800 text-emerald-300">
                ...
                <div className="p-5 flex-1 flex flex-col justify-between space-y-4">
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {restaurant.cuisines.map((c, i) => (
                      <span
                        key={i}
                        className="px-2.5 py-0.5 rounded-lg bg-slate-800 text-slate-300 text-[10px] font-medium"
                      >
                        {c}
                      </span>
                    ))}
                  ...
                  <div className="pt-3 border-t border-slate-800 flex items-center justify-between text-xs text-slate-400">
                    ...
                    <div className="flex items-center gap-1 text-rose-400 font-semibold group-hover:translate-x-1 transition-transform">
                      <span>View Menu</span>
