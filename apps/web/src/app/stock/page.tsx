  ...
  if (user && !hasAccess) {
    return (
      <div className="min-h-[60vh] flex items-center justify-center p-4">
        <div className="text-center space-y-4 max-w-md p-8 rounded-3xl border border-slate-200">
          <ShieldCheck className="w-12 h-12 text-slate-600 mx-auto" />
          <h1 className="text-lg font-bold text-slate-900">Stock Module Access Restricted</h1>
    ...
    <div className="min-h-screen pb-24 bg-slate-50 text-slate-900">
          ...
          <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
            <div className="space-y-3 max-w-2xl">
              <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-emerald-50 border border-emerald-200 text-emerald-700 text-xs font-semibold">
                ...
                <span>Multi-Tenant FIFO Engine</span>
              </div>
            ...
            <div className="flex flex-col sm:flex-row gap-3 w-full md:w-auto">
              ...
              <button
                ...
                className="px-5 py-3 rounded-2xl bg-white border border-slate-200 hover:bg-slate-50 text-slate-700 font-bold text-sm flex items-center justify-center gap-2 transition-all"
              >
      ...
      {activeTab === 'BATCHES' && (
        <div className="space-y-4">
          <div className="p-4 rounded-3xl border border-slate-200 bg-white flex flex-col sm:flex-row gap-4 items-center justify-between">
            <div className="relative w-full sm:w-80">
              ...
              <input
                ...
                className="w-full bg-slate-50 border border-slate-200 rounded-xl pl-9 pr-3 py-2 text-xs text-slate-900 placeholder-slate-500 outline-none focus:border-emerald-500"
              />
            </div>
            <div className="text-xs text-slate-500">
              Batches automatically ordered in <span className="font-mono text-emerald-600 font-bold">FIFO by Expiry Date</span>
            </div>
          </div>
                  ...
                  {filteredBatches.map((b) => {
                    ...
                    return (
                      <tr key={b.id} className="hover:bg-slate-100/40 transition-colors">
                        <td className="p-3.5">
                          <div className="font-bold text-slate-700">{b.productName}</div>
                          <div className="text-[11px] text-emerald-600 font-mono">{b.batchNumber}</div>
                          <div className="text-[11px] text-slate-500">{b.supplierName || 'Direct Intake'}</div>
                        </td>
                        <td className="p-3.5">
                          <div className="flex items-center gap-2">
                            <span className="font-mono font-bold text-base text-slate-900">{b.currentQuantity}</span>
                            <span className="text-[10px] text-slate-500">/ {b.initialQuantity}</span>
                          </div>
                          {b.isLowStock && (
                            <span className="inline-block mt-0.5 px-2 py-0.5 rounded bg-amber-50 text-amber-700 text-[10px] font-mono font-bold">
                              LOW STOCK
                            </span>
                          )}
                        </td>
                        <td className="p-3.5 font-mono text-slate-600">৳{b.purchaseCost.toFixed(2)}</td>
                        <td className="p-3.5 font-mono text-emerald-600 font-bold">৳{b.sellingPrice.toFixed(2)}</td>
                        <td className="p-3.5 font-mono text-indigo-700">৳{b.wholesalePrice.toFixed(2)}</td>
                        <td className="p-3.5">
                        ...
                        <td className="p-3.5">
                          ...
                          <div className="text-[10px] text-slate-500 mt-0.5">
                            {b.daysUntilExpiry <= 30 ? (
                              <span className="text-rose-600 font-bold">Critical: {b.daysUntilExpiry}d left</span>
                            ) : b.daysUntilExpiry <= 90 ? (
                              <span className="text-amber-600 font-semibold">{b.daysUntilExpiry} days left</span>
                            ) : (
                              <span className="text-slate-500">{b.daysUntilExpiry} days left</span>
                            )}
                          </div>
                        </td>
                      </tr>
      ...
      {activeTab === 'POS' && (
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          <div className="lg:col-span-7 p-6 rounded-3xl border border-slate-200 bg-white space-y-5">
            <div className="flex items-center gap-3 border-b border-slate-200 pb-3">
              <Receipt className="w-5 h-5 text-emerald-600" />
              <div>
                <h3 className="text-sm font-bold text-slate-900">Direct Stock Sale & Memo Generator</h3>
                <p className="text-xs text-slate-500">Deducts stock via FIFO and calculates exact net margin.</p>
              </div>
            </div>
            ...
            <form onSubmit={handleExecutePosSale} className="space-y-4 text-xs">
              ...
              <div className="space-y-1">
                <label className="text-slate-600 font-bold">Select Product / Dealing Batch</label>
                <select
                  value={posProductId}
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1">
                  <label className="text-slate-600 font-bold">Sale Quantity (Units)</label>
                  <input
                    type="number"
                    min={1}
                    value={posQty}
                    onChange={(e) => setPosQty(parseInt(e.target.value, 10) || 1)}
                    className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-2 text-xs text-slate-900 outline-none focus:border-emerald-500 font-mono"
                  />
                </div>

                <div className="space-y-1">
                  <label className="text-slate-600 font-bold">Pricing Mode</label>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1">
                  <label className="text-slate-600 font-bold">Customer / Shop Name</label>
                  <input
                    type="text"
                ...
                <div className="space-y-1">
                  <label className="text-slate-600 font-bold">Customer Phone</label>
                  <input
                    type="text"
              ...
              <button
                ...
                className="w-full py-2.5 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-xs shadow-emerald-600/30 transition-all flex items-center justify-center gap-2"
              >
          ...
          <div className="lg:col-span-5 p-6 rounded-3xl border border-slate-200 bg-white space-y-4">
            <div className="flex items-center justify-between border-b border-slate-200 pb-3">
              ...
              {lastReceipt && (
                <button
                  ...
                  className="px-2.5 py-1 rounded-lg bg-slate-100 text-slate-600 hover:text-white text-[11px] flex items-center gap-1"
                >
              ...
              <div className="space-y-4 text-xs font-mono">
                <div className="p-3 rounded-xl bg-white border border-slate-200 space-y-1">
                  <div className="flex justify-between">
                    ...
                    <span className="text-emerald-600 font-bold">{lastReceipt.receiptNumber}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-slate-500">Customer:</span>
                    <span className="text-slate-700">{lastReceipt.customerName}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-slate-500">Mode:</span>
                    <span className="text-indigo-700">{lastReceipt.saleType}</span>
                  </div>
                </div>
                ...
                <div className="space-y-1 text-[11px]">
                  ...
                  <div className="flex justify-between text-base font-bold text-white pt-1 border-t border-slate-200">
                    <span>Payable Total:</span>
                    <span className="text-emerald-600 font-mono">৳{lastReceipt.totalAmount.toFixed(2)}</span>
                  </div>
                </div>

                <div className="p-3 rounded-xl bg-emerald-950/30 border border-emerald-200 text-emerald-700 text-[11px] space-y-1">
                  <div className="flex justify-between">
      ...
      {activeTab === 'ALERTS' && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-xs">
          ...
          <div className="p-5 rounded-3xl border border-slate-200 bg-white space-y-4">
            <div className="flex items-center gap-2 border-b border-slate-200 pb-3">
              <Clock className="w-4 h-4 text-rose-600" />
              <h3 className="font-bold text-slate-900">Expiring Batches (&lt;90 Days Warning)</h3>
            </div>
              ...
              <div className="space-y-2.5">
                {alerts?.expiringBatches.map((b) => (
                  ...
                  >
                    <div>
                      <div className="font-bold text-slate-700">{b.productName}</div>
                      <div className="font-mono text-[11px] text-slate-500">{b.batchNumber} • {b.currentQuantity} units left</div>
                    </div>
          ...
          <div className="p-5 rounded-3xl border border-slate-200 bg-white space-y-4">
            <div className="flex items-center gap-2 border-b border-slate-200 pb-3">
              ...
              <h3 className="font-bold text-slate-900">Sales Velocity & Reorder Recommendations</h3>
            </div>
            ...
            {alerts?.reorderSuggestions.length === 0 ? (
              ...
              <div className="space-y-2.5">
                {alerts?.reorderSuggestions.map((r, idx) => (
                  ...
                  >
                    <div>
                      <div className="font-bold text-slate-700">{r.productName}</div>
                      <div className="text-[11px] text-slate-500">
                        Sales Velocity: <span className="font-mono text-emerald-600 font-bold">{r.dailySalesVelocity} units/day</span>
                      </div>
                    </div>
                    <div className="text-right">
                      <span className="px-2.5 py-1 rounded-full bg-indigo-50 text-indigo-700 border border-indigo-200 font-mono text-[10px] font-bold">
                        Reorder: +{r.recommendedOrderQty} units
                      </span>
                      <div className="text-[10px] text-rose-600 mt-1 font-mono">{r.currentStockDaysLeft} days left</div>
                    </div>
                  </div>
                ))}
              </div>
      ...
      {intakeModalOpen && (
        <div className="fixed inset-0 z-50 bg-white backdrop-blur-sm flex items-center justify-center p-4">
          <div className="max-w-lg w-full p-6 rounded-3xl border border-emerald-200 bg-white space-y-5 shadow-2xl">
            <div className="flex items-center justify-between border-b border-slate-200 pb-3">
              <div>
                <h3 className="text-base font-bold text-slate-900">New Stock Intake / Batch Registration</h3>
                <p className="text-xs text-slate-500">Record purchase cost, selling price, and expiry tracking.</p>
              </div>
              <button onClick={() => setIntakeModalOpen(false)} className="text-slate-500 hover:text-white">✕</button>
            </div>

            <form onSubmit={handleCreateBatch} className="space-y-4 text-xs">
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1">
                  <label className="text-slate-600 font-bold">Initial Intake Quantity</label>
                  <input
                    type="number"
                    min={1}
                ...
                <div className="space-y-1">
                  <label className="text-slate-600 font-bold">Purchase Cost (৳)</label>
                  <input
                    type="number"
                    step="0.1"
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1">
                  <label className="text-slate-600 font-bold">Retail Selling Price (৳)</label>
                  <input
                    type="number"
                    step="0.1"
                ...
                <div className="space-y-1">
                  <label className="text-slate-600 font-bold">Wholesale Rate (৳)</label>
                  <input
                    type="number"
                    step="0.1"
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div className="space-y-1">
                  <label className="text-slate-600 font-bold">Supplier / Depot Name</label>
                  <input
                    type="text"
              ...
              <div className="flex items-center justify-end gap-2 pt-3 border-t border-slate-200">
                <button
                  ...
                  className="px-4 py-2 rounded-xl bg-slate-100 hover:bg-slate-700 text-xs text-slate-600 font-medium"
                >
                  Cancel
                </button>
                <button
                  type="submit"
                  className="px-5 py-2 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-xs text-white font-bold shadow-emerald-600/30"
                >
                  Confirm Intake
                </button>
              </div>
