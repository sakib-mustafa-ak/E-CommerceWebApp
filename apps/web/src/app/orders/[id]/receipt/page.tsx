    ...
    return (
      <div className="min-h-[60vh] flex items-center justify-center p-4 text-center">
        <div className="space-y-3">
          ...
          <Link href="/" className="px-4 py-2 bg-slate-800 text-slate-300 rounded-xl text-xs">
            Back to Home
          </Link>
        </div>
      </div>
    ...
    <div className="max-w-3xl mx-auto px-4 py-8 space-y-6">
      ...
      <div className="flex justify-between items-center print:hidden">
        ...
        <button
          ...
          className="px-4 py-2 bg-sky-500 hover:bg-sky-400 text-white rounded-xl text-xs font-bold flex items-center gap-2 shadow-lg shadow-sky-500/20"
        >
      ...
      <div className="glass-panel p-8 rounded-3xl border border-slate-800 bg-slate-900/90 text-white space-y-6 print:bg-white print:text-black print:border-none print:shadow-none">
        {/* Invoice Header */}
        <div className="flex justify-between items-start border-b border-slate-800 print:border-slate-300 pb-6">
          <div>
            <div className="text-xl font-extrabold text-sky-400 print:text-sky-700 font-mono tracking-wider">
          ...
          <div className="text-right">
            <span className="text-[10px] uppercase font-bold font-mono px-2 py-0.5 rounded bg-sky-500/10 text-sky-400 border border-sky-500/20 print:bg-slate-100 print:text-slate-800">
              Customer Memo
            </span>
        ...
        <div className="grid grid-cols-2 gap-4 text-xs">
          <div>
            ...
            <div className="font-bold text-white print:text-black mt-0.5">{receipt.customerName}</div>
            {receipt.customerPhone && (
              <div className="text-slate-400 print:text-slate-600 font-mono">{receipt.customerPhone}</div>
            )}
            {receipt.customerEmail && (
              <div className="text-slate-400 print:text-slate-600">{receipt.customerEmail}</div>
            )}
          </div>
          <div className="text-right">
            <span className="text-slate-500 print:text-slate-600 block">Delivery & Payment:</span>
            <div className="text-slate-300 print:text-slate-700 mt-0.5">{receipt.deliveryAddress}</div>
            <div className="font-mono text-[11px] text-sky-400 print:text-sky-700 font-bold mt-1">
        ...
        <div className="border border-slate-800 print:border-slate-300 rounded-2xl overflow-hidden text-xs">
          <table className="w-full text-left">
            <thead className="bg-slate-950/60 print:bg-slate-100 border-b border-slate-800 print:border-slate-300 text-slate-400 print:text-slate-700">
              <tr>
            ...
            <tbody className="divide-y divide-slate-800/60 print:divide-slate-200">
              {receipt.items.map((item: any) => (
                <tr key={item.id}>
                  <td className="p-3">
                    <div className="font-bold text-white print:text-black">{item.productName}</div>
        ...
        <div className="flex justify-end pt-2">
          <div className="w-64 space-y-1.5 text-xs">
            ...
            <div className="flex justify-between text-base font-bold text-white print:text-black pt-2 border-t border-slate-800 print:border-slate-300">
        ...
        {/* Footer Guarantee */}
        <div className="pt-6 border-t border-slate-800 print:border-slate-300 text-center text-[10px] text-slate-500 print:text-slate-600">
