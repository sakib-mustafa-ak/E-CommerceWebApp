...
export default function CommunityHubPage() {
  ...
  return (
    <div className="min-h-screen pb-24 bg-slate-50 text-slate-900">
      {/* Hero Banner */}
      <div className="relative overflow-hidden bg-gradient-to-b from-cyan-50 via-white to-slate-50 border-b border-slate-200 pt-10 pb-16">
          ...
          <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
            ...
            <div className="flex flex-col sm:flex-row gap-3 w-full md:w-auto">
              <button
                ...
                className="px-6 py-3.5 rounded-2xl bg-[#0F5B78] hover:bg-[#0d4e68] text-white font-bold text-sm shadow-md shadow-[#0F5B78]/20 flex items-center justify-center gap-2 transition-all transform active:scale-95"
              >
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-8">
        ...
        {posts.length === 0 ? (
          <div className="p-12 text-center rounded-3xl border border-slate-200 bg-white space-y-3">
            ...
            <p className="text-slate-600 font-semibold">No classifieds match your search filter.</p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {posts.map((post) => (
              <div
                key={post.id}
                className="group p-6 rounded-3xl border border-slate-200 hover:border-indigo-500/50 bg-white hover:bg-white transition-all duration-300 shadow-xl flex flex-col justify-between space-y-4"
              >
                <div className="space-y-3">
                  {/* Top Metadata Tags */}
                  <div className="flex flex-wrap items-center justify-between gap-2">
                    <span className="px-3 py-1 rounded-full bg-indigo-50 border border-indigo-200 text-indigo-700 text-[11px] font-semibold">
                      {post.category.replace('_', ' ')}
                    </span>

                    {post.linkedSector && (
                      <span className="px-2.5 py-0.5 rounded-lg bg-slate-100 text-slate-600 text-[10px] font-mono">
                  ...
                  {/* Price / Budget Tag */}
                  {post.priceBdt && (
                    <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-xl bg-slate-50 border border-slate-200 text-xs font-mono font-bold text-emerald-600">
                  ...
                  {post.tags && post.tags.length > 0 && (
                    <div className="flex flex-wrap gap-1.5 pt-1">
                      {post.tags.map((t, idx) => (
                        <span
                          key={idx}
                          className="px-2 py-0.5 rounded-md bg-slate-50 text-slate-500 text-[10px] font-mono border border-slate-200"
                        >
                          #{t}
                        </span>
                      ))}
                ...
                <div className="pt-4 border-t border-slate-200 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 text-xs text-slate-500">
                  ...
                  <Link
                    ...
                    className="px-4 py-2 rounded-xl bg-slate-100 hover:bg-indigo-600 text-white font-semibold text-xs flex items-center gap-1.5 transition-colors group-hover:shadow-md"
                  >
          ...
          <div className="relative w-full max-w-2xl rounded-3xl border border-slate-200 bg-white p-6 sm:p-8 shadow-2xl space-y-6">
            <div className="flex justify-between items-start">
              ...
              <button
                ...
                className="p-1.5 rounded-xl bg-slate-100 text-slate-500 hover:text-white"
              >
                ✕
              </button>
            </div>
            ...
            {submitSuccess ? (
              <div className="p-6 rounded-2xl bg-emerald-50 border border-emerald-800 text-emerald-700 text-center space-y-2">
              ...
              <form onSubmit={handleCreatePost} className="space-y-4 text-xs">
                <div>
                  <label className="text-slate-600 font-semibold block mb-1">Classified Title</label>
                  <input
                    type="text"
                    required
                    value={newTitle}
                    ...
                    className="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-slate-900 focus:border-indigo-500 focus:outline-none"
                  />
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <div>
                    <label className="text-slate-600 font-semibold block mb-1">Related Sector</label>
                    <select
                      value={newSector}
                ...
                <div>
                  <label className="text-slate-600 font-semibold block mb-1">Detailed Content</label>
                  <textarea
                    rows={4}
                    required
                    value={newContent}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                  <div>
                    <label className="text-slate-600 font-semibold block mb-1">Location / Route</label>
                    <input
                      type="text"
                      value={newLocation}
                      onChange={(e) => setNewLocation(e.target.value)}
                      placeholder="e.g. Dhanmondi, Dhaka"
                      className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl text-slate-900 focus:border-indigo-500 focus:outline-none"
                    />
                  </div>

                  <div>
                    <label className="text-slate-600 font-semibold block mb-1">Budget / Price (৳)</label>
                    <input
                      type="number"
                      value={newPrice}
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-3.5 py-2 bg-slate-50 border border-slate-200 rounded-xl text-slate-900 focus:border-indigo-500 focus:outline-none"
                  />
                </div>
                ...
                <div className="pt-2 flex justify-end gap-3">
                  <button
                    type="button"
                    onClick={() => setIsModalOpen(false)}
                    className="px-4 py-2.5 rounded-xl bg-slate-100 text-slate-600 hover:bg-slate-700 font-semibold"
                  >
                    Cancel
                  </button>
                  <button
                    type="submit"
                    disabled={submitting}
                    className="px-6 py-2.5 rounded-xl bg-gradient-to-r from-indigo-600 to-sky-600 hover:from-indigo-500 hover:to-sky-500 text-white font-bold transition-all shadow-indigo-950 disabled:opacity-50"
                  >
