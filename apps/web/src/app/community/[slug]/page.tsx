  ...
  if (!post) {
    return (
      <div className="min-h-screen bg-slate-950 flex flex-col items-center justify-center p-4 text-center">
        ...
        <h2 className="text-xl font-bold text-white mb-2">Classified Post Not Found</h2>
        <Link href="/community" className="text-indigo-400 hover:underline text-sm">
    ...
    <div className="min-h-screen pb-24 bg-slate-950 text-slate-100">
      ...
      <div className="bg-slate-900/80 border-b border-slate-800 py-6">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 flex items-center justify-between">
          <Link
            href="/community"
            className="px-3.5 py-2 rounded-xl bg-slate-800 hover:bg-slate-700 text-white text-xs font-semibold flex items-center gap-2 transition-colors"
          >
        ...
        <div className="p-6 sm:p-8 rounded-3xl border border-slate-800 bg-slate-900/90 shadow-2xl space-y-6">
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 pb-4 border-b border-slate-800">
            <div className="flex items-center gap-2">
              ...
              <span className="px-3 py-1 rounded-full bg-emerald-500/20 text-emerald-300 border border-emerald-500/40 text-xs font-semibold flex items-center gap-1">
          ...
          <div className="space-y-3">
            ...
            {post.priceBdt && (
              <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-2xl bg-slate-950 border border-slate-800 text-sm font-mono font-black text-emerald-400">
          ...
          {/* Detailed Content */}
          <div className="p-6 rounded-2xl bg-slate-950/70 border border-slate-800/80 text-sm text-slate-200 leading-relaxed whitespace-pre-wrap font-sans">
            {post.content}
          </div>
          ...
          {post.tags && post.tags.length > 0 && (
            <div className="flex flex-wrap gap-2 pt-2">
              {post.tags.map((t, idx) => (
                <span
                  key={idx}
                  className="px-3 py-1 rounded-lg bg-slate-800 text-slate-300 text-xs font-mono"
                >
                  #{t}
                </span>
              ))}
          ...
          {post.linkedSector && (
            <div className="p-4 rounded-2xl bg-gradient-to-r from-indigo-950/60 via-slate-900 to-sky-950/60 border border-indigo-500/30 flex items-center justify-between gap-4">
              <div className="flex items-center gap-3">
                <Store className="w-5 h-5 text-indigo-400 shrink-0" />
                <div className="text-xs">
                  <span className="text-slate-400 block font-mono">Linked Commerce Sector</span>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs">
            ...
            <div className="p-4 rounded-2xl bg-slate-950 border border-slate-800 space-y-2">
              ...
              {post.authorPhone ? (
                ...
                <div className="text-slate-400">Phone not provided by poster.</div>
              )}

              {post.authorEmail && (
                <a
                  href={`mailto:${post.authorEmail}`}
                  className="px-4 py-2 bg-slate-800 hover:bg-slate-700 text-slate-200 font-semibold text-xs rounded-xl flex items-center justify-center gap-1.5 transition-colors"
                >
