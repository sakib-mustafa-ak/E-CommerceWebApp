...
export default function PublicCheckoutPage() {
  ...
  if (orderComplete) {
    return (
      <div className="max-w-2xl mx-auto px-4 py-16 text-center space-y-6">
        <div className="glass-panel p-8 rounded-3xl border border-emerald-500/30 bg-slate-900/90 space-y-6 shadow-2xl">
          ...
          <div>
            ...
            <h1 className="text-2xl font-bold text-white mt-1">Thank You For Your Order!</h1>
          ...
          {orderComplete.digitalDownloadTokens && orderComplete.digitalDownloadTokens.length > 0 && (
            <div className="p-4 rounded-2xl bg-purple-500/10 border border-purple-500/30 space-y-3 text-left">
              ...
              <div className="space-y-2">
                {orderComplete.digitalDownloadTokens.map((dt: any) => (
                  <div
                    key={dt.token}
                    className="p-3 bg-slate-950/80 rounded-xl flex items-center justify-between text-xs"
                  >
                    <div>
                      <div className="font-bold text-white">{dt.productName}</div>
          ...
          <div className="pt-4 border-t border-slate-800 flex justify-center gap-3">
            <Link
              ...
              className="px-4 py-2 rounded-xl bg-slate-800 text-sky-400 font-bold text-xs hover:bg-slate-700"
            >
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
      <div>
        <h1 className="text-2xl font-bold text-white">Public Customer Checkout</h1>
        <p className="text-xs text-slate-400 mt-1">
      ...
      {errorMessage && (
        <div className="p-4 rounded-2xl bg-rose-500/10 border border-rose-500/20 text-rose-300 text-xs flex items-center gap-2 font-semibold">
          <AlertTriangle className="w-4 h-4" /> {errorMessage}
        </div>
      )}

      {cart.length === 0 ? (
        <div className="glass-panel p-12 text-center text-slate-500 text-xs rounded-3xl border border-dashed border-slate-800 space-y-3">
          <ShoppingBag className="w-8 h-8 mx-auto text-slate-600" />
          <div className="font-bold text-slate-300">Your cart is currently empty.</div>
          <Link
            href="/"
        ...
        <form onSubmit={handleCheckoutSubmit} className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="lg:col-span-7 space-y-6">
            ...
            <div className="glass-panel p-6 rounded-3xl border border-slate-800 bg-slate-900/80 space-y-4">
              <div className="flex items-center justify-between border-b border-slate-800 pb-3">
                <h3 className="text-sm font-bold text-white flex items-center gap-2">
                  <UserCheck className="w-4 h-4 text-sky-400" /> Customer Information
                </h3>
                {user ? (
                  <span className="text-xs text-emerald-400 font-mono">Logged in as {user.name}</span>
                ) : (
                  <span className="text-xs text-amber-400 font-mono">Guest Checkout Mode</span>
                )}
              </div>
                ...
                <div className="space-y-3 text-xs">
                  <div>
                    ...
                    <input
                      ...
                      className="w-full px-3 py-2 rounded-xl bg-slate-800 border border-slate-700 text-white font-sans focus:outline-none focus:border-sky-500"
                    />
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      ...
                      <input
                        ...
                        className="w-full px-3 py-2 rounded-xl bg-slate-800 border border-slate-700 text-white font-mono focus:outline-none focus:border-sky-500"
                      />
                    </div>
                    <div>
            ...
            {!isAllDigital && (
              <div className="glass-panel p-6 rounded-3xl border border-slate-800 bg-slate-900/80 space-y-4">
                <h3 className="text-sm font-bold text-white flex items-center gap-2">
                  <Truck className="w-4 h-4 text-sky-400" /> Physical Delivery Address
                </h3>
            ...
            <div className="glass-panel p-6 rounded-3xl border border-slate-800 bg-slate-900/80 space-y-4">
              <h3 className="text-sm font-bold text-white flex items-center gap-2">
                <CreditCard className="w-4 h-4 text-sky-400" /> Select Payment Method
              </h3>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 text-xs">
                ...
                <div
                  ...
                  className={`p-3.5 rounded-2xl border transition-all ${
                    hasCodDisabledItem || isAllDigital
                      ...
                      : paymentMethod === 'COD'
                      ? 'bg-sky-500/20 border-sky-500 text-white font-bold cursor-pointer'
                      : 'bg-slate-800/60 border-slate-700 text-slate-300 hover:border-slate-600 cursor-pointer'
                  }`}
                >
                  <div className="font-bold">Cash on Delivery</div>
                  <div className="text-[11px] text-slate-400 mt-0.5">Pay upon delivery</div>
                  {(hasCodDisabledItem || isAllDigital) && (
                    <div className="text-[10px] text-amber-400 mt-1">Not available for this cart</div>
                  )}
                </div>
                ...
                >
                  ...
                  <div className="text-[11px] text-slate-400 mt-0.5">Instant gateway</div>
                </div>
                ...
                >
                  ...
                  <div className="text-[11px] text-slate-400 mt-0.5">Secure card payment</div>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="lg:col-span-5 space-y-6">
            <div className="glass-panel p-6 rounded-3xl border border-slate-800 bg-slate-900/80 space-y-4">
              <h3 className="text-sm font-bold text-white flex items-center justify-between">
                ...
                <span className="text-xs text-sky-400 font-mono">Retail Base MRP</span>
              </h3>
              ...
              <button
                type="submit"
                disabled={submitting}
                className="w-full py-3 rounded-2xl bg-sky-500 hover:bg-sky-400 text-white font-extrabold text-sm shadow-xl shadow-sky-500/25 transition-all flex items-center justify-center gap-2 disabled:opacity-50"
              >
